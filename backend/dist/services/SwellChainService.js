...
exports.SwellChainService = void 0;
const logger_1 = require("../utils/logger");
class SwellChainService {
    prisma;
    redis;
    swellchainConfig;
    avsContracts = new Map();
    ...
    async getAVSMetrics() {
        try {
            ...
            if (cached) {
                logger_1.logger.debug('AVS metrics served from cache');
                return JSON.parse(cached);
            }
            logger_1.logger.avs('Fetching real-time AVS metrics from Swellchain');
     ...
     */
    async getCrossChainPositions(userAddress) {
        try {
            ...
            logger_1.logger.bridge('Fetching cross-chain positions', { userAddress });
            ...
            logger_1.logger.error('Error fetching cross-chain positions', error);
            throw error;
        }
    }
    /**
     ...
     */
    async executeBridgeOperation(operation) {
        try {
            logger_1.logger.bridge('Executing bridge operation', operation);
            ...
            logger_1.logger.bridge('Bridge operation completed', {
                id: bridgeOp.id,
            ...
            logger_1.logger.error('Error executing bridge operation', error);
            throw error;
        }
    }
    /**
    ...
    async getProtocolData() {
        try {
            ...
            logger_1.logger.analytics('Fetching Swellchain protocol data');
            ...
            logger_1.logger.error('Error fetching protocol data', error);
            throw error;
        }
    }
    /**
    ...
    async getRecentTransactions(limit = 100) {
        try {
            ...
            logger_1.logger.analytics('Fetching recent Swellchain transactions');
            ...
            logger_1.logger.error('Error fetching recent transactions', error);
            throw error;
        }
    }
    /**
    ...
    async getValidatorPerformance(validatorAddress) {
        try {
            ...
            logger_1.logger.error('Error fetching validator performance', error);
            throw error;
        }
    }
    // Private helper methods for data fetching
    async fetchAVSData(avsId) {
    ...
    async fetchUserCrossChainPositions(userAddress) {
    ...
    async validateBridgeOperation(operation) {
    ...
    async performBridgeTransaction(operation) {
    ...
    async fetchSwellchainProtocols() {
    ...
    async fetchRecentTransactions(limit) {
    ...
    async fetchValidatorPerformance(validatorAddress) {
...
exports.SwellChainService = SwellChainService;
//# sourceMappingURL=SwellChainService.js.map