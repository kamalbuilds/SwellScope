...
class RiskService {
    ...
    async getRiskMetrics(userAddress) {
        try {
            ...
            if (cached) {
                logger_1.logger.debug('Risk metrics served from cache', { userAddress });
                return JSON.parse(cached);
            }
            logger_1.logger.risk('Calculating risk metrics', { userAddress });
            const [overallRisk, slashingRisk, liquidityRisk, concentrationRisk, validatorRisks, avsRisks] = await Promise.all([
            ...
            logger_1.logger.risk('Risk metrics calculated', {
                userAddress,
            ...
            logger_1.logger.error('Error calculating risk metrics', error);
            throw error;
        }
    }
    /**
    ...
    async getRiskAlerts(userAddress) {
        try {
            ...
            logger_1.logger.risk('Fetching risk alerts', { userAddress });
            const alerts = [];
            ...
            logger_1.logger.error('Error fetching risk alerts', error);
            throw error;
        }
    }
    /**
     ...
     */
    async calculateOverallRisk(userAddress) {
        ...
        for (const position of positions) {
            ...
            const positionRisk = Math.sqrt(Math.pow(protocolRisk.score, 2) +
                Math.pow(validatorRisk.score, 2) +
                Math.pow(avsRisk.score, 2)) / Math.sqrt(3);
     ...
     */
    async calculateSlashingRisk(userAddress) {
        const positions = await this.getUserStakingPositions(userAddress);
        let totalStaked = 0;
        ...
        for (const position of positions) {
            ...
            const positionSlashingProbability = Math.min(historicalProbability * currentRiskFactors.multiplier + avsSlashingRisk.baseProbability, 0.1 // Cap at 10% probability
            );
     ...
     */
    async calculateLiquidityRisk(userAddress) {
        const positions = await this.getUserStakingPositions(userAddress);
        let totalStaked = 0;
     ...
     */
    async calculateConcentrationRisk(userAddress) {
        const positions = await this.getUserStakingPositions(userAddress);
        const protocolDistribution = new Map();
        const operatorDistribution = new Map();
        const avsDistribution = new Map();
        let totalValue = 0;
        positions.forEach(position => {
            totalValue += position.value;
            protocolDistribution.set(position.protocol, (protocolDistribution.get(position.protocol) || 0) + position.value);
            operatorDistribution.set(position.operator, (operatorDistribution.get(position.operator) || 0) + position.value);
            avsDistribution.set(position.avs, (avsDistribution.get(position.avs) || 0) + position.value);
        });
     ...
     */
    calculateHHI(distribution, total) {
        let hhi = 0;
     ...
     */
    async getValidatorRisks(userAddress) {
        const positions = await this.getUserStakingPositions(userAddress);
        const validatorRisks = [];
        for (const position of positions) {
            const validatorRisk = await this.getValidatorRisk(position.validator);
            validatorRisks.push({
    ...
    async getUserStakingPositions(userAddress) {
    ...
    async getValidatorRisk(validator) {
        return {
            score: 0.2,
            slashingHistory: [],
            performance: 0.98,
    ...
    async getValidatorSlashingHistory(validator) {
        return { slashingEvents: 0, totalEpochs: 1000 };
    }
    async getCurrentRiskFactors(validator, avs) {
        return { multiplier: 1.0 };
    }
    async getProtocolLiquidity(protocol) {
        return { available: 1000 };
    }
    async getWithdrawalDelay(protocol, validator) {
        return 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds
    }
    async getHighRiskValidators(userAddress) {
        // This would fetch real high-risk validators from Swellchain data
        // For now, returning empty array but with proper type
        return [];
    }
    async getRecentSlashingEvents(userAddress, days) {
        return [];
    }
    async getAVSSlashingRisk(avs) {
