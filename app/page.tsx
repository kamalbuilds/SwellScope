...
import AVSPerformance from './components/AVSPerformance';
import CrossChainPositions from './components/CrossChainPositions';
import AlertPanel from './components/AlertPanel';
import LoadingSpinner from './components/LoadingSpinner';

// Import hooks
import { useSwellScopeData } from './hooks/useSwellScopeData';
import { useRiskAssessment } from './hooks/useRiskAssessment';
import { usePortfolioManagement } from './hooks/usePortfolioManagement';

// Import types
import { AnalyticsData, RiskMetrics, PortfolioData, AVSMetrics } from './types';

interface SwellScopeDashboardProps {}

export default function SwellScopeDashboard({}: SwellScopeDashboardProps) {
  const { address, isConnected } = useAccount();
  const [socket, setSocket] = useState<Socket | null>(null);
  const [activeTab, setActiveTab] = useState<'overview' | 'risk' | 'portfolio' | 'avs' | 'bridge'>('overview');
  ...
  const { 
    ...
    rebalancePortfolio, 
    isRebalancing 
  } = usePortfolioManagement(address);
  ...
  useEffect(() => {
    if (isConnected && address) {
      ...
      newSocket.on('connect', () => {
        ...
        newSocket.emit('subscribe', { type: 'risk_updates', address });
        newSocket.emit('subscribe', { type: 'portfolio_updates', address });
        newSocket.emit('subscribe', { type: 'avs_updates' });
      ...
      newSocket.on('portfolio:rebalance', (data) => {
        console.log('Portfolio rebalance completed:', data);
        // Update portfolio data
      });

      newSocket.on('alert:triggered', (data) => {
        console.log('Risk alert triggered:', data);
  ...
  useEffect(() => {
    if (!isLoadingAnalytics && !isLoadingRisk) {
      setIsLoading(false);
    }
  }, [isLoadingAnalytics, isLoadingRisk]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {riskAlerts && riskAlerts.length > 0 && (
          <AlertPanel alerts={riskAlerts} className="mb-8" />
        )}
        ...
        <div className="space-y-8">
          ...
          {activeTab === 'risk' && (
            <RiskMonitor 
              ...
              isLoading={isLoadingRisk}
              detailed={true}
              onUpdateProfile={updateRiskProfile}
            />
          )}
